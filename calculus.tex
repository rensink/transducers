...
\end{itemize}
%
We will explain the purpose and functioning of control programs by first restricting to the fragment without refinement. For the semantics, we adapt the approach of \cite{CCS-Priority-Choice} to the setting of this paper. (See \Cref{sec:related} for a brief discussion of the adaptations.)
...
In a control program of the form $\rC_1\try \rC_2$, the operand $\rC_2$ is ``activated'' if and only if none of the actions initially invoked by $\rC_1$ are enabled in the target system; the control program $\alap \rC_1$ terminates in that case. Hence action invocation by, as well as termination of, a control program is in general \emph{conditional} on the failure of other, previously attempted actions. This effect is captured by the following predicates:
%
\begin{itemize}
...
%
\begin{center}
\AxiomC{$\rC_1\invokes X$}
\AxiomC{$\rC_2\trans{a\ifnot Y} \rC'$}
\BinaryInfC{$\rC_1\try \rC_2\displaytrans{a\ifnot(X\cup Y)} \rC'$}
\DisplayProof
%
\quad
%
...
\AxiomC{$\neg (\cS\enables X)$}
\AxiomC{$\cS\trans a \cS'$}
\TrinaryInfC{$\rC\on \cS \displaytrans a \rC'\on \cS'$}
\DisplayProof
\end{center}
